...
public class CodeMirrorDocument extends AbstractEmbeddedDocument {
    ...
    public CodeMirrorDocument(final CMDocumentOverlay documentOverlay,
        ...
        documentOverlay.getEditor().on(BEFORE_CHANGE, new CMEditorOverlay.EventHandlerMultipleParameters<JavaScriptObject>() {
            @Override
            public void onEvent(final JsArray<JavaScriptObject> params) {

                // first parameter is editor instance, second is the change
                final CMBeforeChangeEventOverlay change = params.get(1).cast();
